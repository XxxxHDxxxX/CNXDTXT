...
![](images/fbe7f3585c17de97ef2450c4592d5e3c0b109cd856e7cbe31ad028d0729d8e82.jpg)
...
public class Student   
{ public Student(string name,int age,char gender,int chinese,int math,int english) { this.Name $=$ name; this.Age $=$ age; this.Gender $=$ gender; this.Chinese $\equiv$ chinese; this.Math $=$ math; this.English $\equiv$ english; } public Student(string name,int age,char gender):this(name,age,gender,0,0,0) { //this.Name $=$ name; //this.Age $=$ age; //this.Gender $=$ gender; } public Student(string name,int chines, int math, int english):this(name,0,'c',chines,math,english) { //this.Name $=$ name; //this.Chinese $=$ chinese; //this.Math $=$ math; //this.English $\equiv$ english; }
...
![](images/630b650421f7ae874360551bfe365f3d1ae8fea2b5f0403a6094305f36072d44.jpg)
...
# 一、设计一个 Person 类，其类中包含如下信息：

1. 私有字段，姓名： name ；血型： blood ；  
2. 属性， Name ； Blood ，注意 Blood 只有 A 、 B 、 O 、 AB 这四种，如果输入错误将使用 O 型；  
3. 方法， PrintName 方法将名字打印到控制台显示， PrintBlood 方法将血型打印到控制台显示；  
4. 在 Main 函数中创建一个 Person 对象 p 如下：

Person p=new Person(“ 江湖” ,“AB”) ；然后，使用 PrintName 方法将姓名显示出来。使用 PrintBlood 方法将血型显示出来。

# 二、如右图表述的继承关系，利用面向对象中封装、继承、多态的特性完成以下要求：

1. 编写一个Shape类作为基类，编写子类Triangle ， Rectangle ， Circle 继承于 Shape ；  
2. 每个类中需要包含字段，带保护的属性 ( 如果设置的值小于0 则设置默认值 1) ，构造函数；  
3. 每个子类中都有一个表述身份的方法和一个计算周长的方法，请用多态的思想实现；  
4. 在 Main 函数中实现每个子类的每个方法；

![](images/2a6cb507d6e5a81ab26bab6a7d4ea25994b763ec593c7d3e7ce4c875f645e761.jpg)
